    ...
    return (
        <MyNftContainer filter={activeButton.filter} market={props.market} sweep={activeButton.sweep} openSelect={open}>
            <div className="filter-group">
               ...
               { !props.market && <div className="button-group">
                    <button className="button link-ens"><a target="blank" href="https://ens.vision/"><img src={ensvision} alt="" /></a></button>
                    <button className="button filter" onClick={()=> setActiveButton({...activeButton,filter:activeButton.filter ? false : true})}><img src={props.theme === 'Light Theme' ? filterDark : filterLight} alt="" /></button>
                </div>}
                ...
                { props.market &&  <div style={{marginBottom:'20px'}} >
                 <div className="button-group">
                    <button className="button link-ens"><a target="blank" href="https://ens.vision/"><img src={ensvision} alt="" /></a></button>
                    <button className="button filter" onClick={()=> setActiveButton({...activeButton,filter:activeButton.filter ? false : true})}><img src={props.theme === 'Light Theme' ? filterDark : filterLight} alt="" /></button>
                   <button className="button sweep" onClick={()=> setActiveButton({...activeButton,sweep:activeButton.sweep ? false : true})}><img src={props.theme === 'Light Theme' ? sweepDark : sweepLight} alt="" /></button>
                </div>
                <div ref={selectRef} className="filter-select" onClick={()=> setOpen(open ? false: true)  }>
                <div  className="visible" ><span className="filter-selection" >{selected}</span>
                <div className="icon"><img style={{width:'15px'}} src={ props.theme === 'Dark Theme'? arrowDownLight : arrowDown} alt=""/></div>
                 </div>
                    <div className="filter-option">
                        <div onClick={(e) => {setSelected(e.target.innerText);setOpen(false);}}> Price Low to High</div>
                        <div onClick={(e) => {setSelected(e.target.innerText);setOpen(false);}}>Price High to Low</div>
                    </div>
                </div>
                </div>}
            </div>
                ...
                <div className="myNft" key={index}>
                    ...
                    <ToolBar market={props.market} open={modalOpen === index + 1 && isOpen ? true : false} >
                        ...
                        <img
                            ...
                            src={currentTheme.theme.name === "Dark Theme" ? DotLight : DotDark}
                        />
                        </button>
                        {(modalOpen === index + 1 && isOpen ) &&
                            <div ref={modalRef} className="modal-option">
                                <ul>
                                    <li className="option" onClick={() => optionAction('stacker')}>Stacker</li>
                                    <li className="option" onClick={() => optionAction('sell')}>Sell</li>
                                    <li className="option" onClick={() => optionAction('burn')}>Burn</li>
                                </ul>
                            </div>
                        }
                    </ToolBar>
